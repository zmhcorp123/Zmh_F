...
export function Home() {
  return (
    <>
      ...
      <section><SectionHeader eyebrow="Demo feedback" title="Operators trust repeatable systems" text="Sample testimonials for the public homepage until real client approvals are available." /><div className="grid three">{testimonials.map((item) => <article className="quote" key={item.company}><p>"{item.quote}"</p><strong>{item.name}</strong><span>{item.company}</span></article>)}</div></section>
      <CTA />
    </>
  );
}
